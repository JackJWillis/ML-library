...
## yx dataframes typically require y to be the first variable



### Standardize the x-variables of dataframe. Note, only acts on cardinal variables. 

# Assumes y is the first variable

standardize.x = function(yx) {
  x <- yx[,-1] 
  #standardizing x-variables
  nums <- sapply(x,is.numeric)
  x[,nums]  <- scale(x[,nums], center = TRUE, scale = TRUE)
  #Standardizing can cause some variables to become NA (if they are constant)
  #Hence dropping those variables
