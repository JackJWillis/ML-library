...
* Machine learning is being used to improve the accuracy of targeting across a wide range of fields.
    + For example, search engines are paid whenever a user clicks an ad. Google and Microsoft use machine learning systems to decide which ads to display to each user.
    + In healthcare, the 'HeRO' monitor uses data from existing bedside monitors to create a predictive score for sepsis. In a clinical trial, use of the 'HeRO' score reduced mortality in low birth weight infants in the ICU.
...
Proxy Means Testing
===============
* Identifying the poor is difficult in developing countries. Household welfare is assessed through a tedious and time-consuming consumption survey. It is infeasible to directly calculate consumption for an entire population.
* A popular method for targeting the poor is proxy means testing (PMT) where easily observable household characteristics are used as statistical proxies for consumption.
* Proxy means is not the only targeting method.
    + Community targeting: Communities or local leaders select beneficiaries.
    + Self targeting: Use requirements that are more costly to the rich than to the poor to dissuade the rich from participating.
...
* Regularized linear methods like ridge and LASSO can provide better predictions by reducing overfitting.
* Nonlinear methods like decision trees allow for more flexible functional forms which may be able to capture more exotic relationships.
...
* Our research was divided into an exploratory phase and a validation phase.
    ...
    + In order to avoid over-fitting, we selected a small number of well-performing or representative methods and tested them on 3 holdout countries: South Africa, Iraq and Brazil.
...
Metrics
===============
We think of poverty targeting as a classification problem and focus primarily on two metrics of success:
